    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-red-100">
            <div className="bg-white shadow-lg rounded-lg p-8 max-w-md w-full">
                <h2 className="text-2xl font-bold text-red-600 mb-4 text-center">Đăng ký</h2>
                {message && <div className="bg-green-100 text-green-700 p-2 mb-4 rounded">{message}</div>}
                {error && <div className="bg-red-100 text-red-700 p-2 mb-4 rounded">{error}</div>}
                <form onSubmit={handleRegister}>
                    <div className="mb-4">
                        <label className="block text-gray-700 font-semibold">Tên đăng nhập</label>
                        <input
                            type="text"
                ...
                <p className="mt-4 text-center text-gray-600">
                    Đã có tài khoản?
                    <button onClick={() => navigate('/login')} className="text-red-500 font-bold ml-1 hover:underline">
                        Đăng nhập ngay
                    </button>
                </p>
            </div>
